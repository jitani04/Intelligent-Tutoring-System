...
QA_SAMPLE_SIZE = 20  # how many QA rows we'll eval — must match ragas_eval.py
DISTRACTOR_PASSAGES = 200  # extra non-relevant passages to make retrieval non-trivial
# Gemini free tier text-embedding-004 ≈ 100 RPM. Run serially with pacing.
EMBED_MIN_INTERVAL_SEC = 0.7  # ~85 RPM, safely under the 100 RPM cap
...
async def _embed_with_retry(embedder, text: str) -> list[float]:
    ...
    for attempt in range(7):
        try:
            ...
            if e.response.status_code in (429, 503) and attempt < 6:
...
async def ingest_passages(
    ...
    last = 0.0
    for i, (_, text) in enumerate(passages, start=1):
        elapsed = time.monotonic() - last
        if elapsed < EMBED_MIN_INTERVAL_SEC:
            await asyncio.sleep(EMBED_MIN_INTERVAL_SEC - elapsed)
        vectors.append(await _embed_with_retry(embedder, text))
        last = time.monotonic()
        if i % 25 == 0 or i == len(passages):
            print(f"    {i}/{len(passages)} embedded")
