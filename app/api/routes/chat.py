...
TEACHING_PACING_PROMPT = (
    ...
    "\n\nVisual selection rule: use create_structured_diagram for polished instructional visuals such as "
    "queues, stacks, arrays, linked lists, trees, timelines, cycles, comparisons, and annotated learning "
    "figures. Use create_diagram for Mermaid-native charts such as flowcharts, ERDs, sequence diagrams, "
    "state machines, and system architecture."
    "\n\nNo-duplication rule for tool-rendered cards: when you call generate_quiz, create_structured_diagram, create_diagram, find_image, or "
    "find_resource, the resulting card already shows the full artifact (question + options + explanation for quiz, "
    "the rendered diagram, the image with caption, the resource title + link + reason). Do NOT also write "
